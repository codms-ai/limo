...
class LimoMissionUnified:
    def __init__(self):
        ...
        # ✅ 현재 토픽 리스트 기준으로 고정 (Astra 카메라용으로 변경됨)
        self.use_compressed = rospy.get_param("~use_compressed", True)
        if self.use_compressed:
            self.img_sub = rospy.Subscriber(
                "/camera/rgb/image_raw/compressed", CompressedImage, self.image_callback, queue_size=1
            )
        else:
            self.img_sub = rospy.Subscriber(
        ...
        rospy.loginfo("Unified node started. image=%s, scan=/scan",
                      "/camera/rgb/image_raw/compressed" if self.use_compressed else "/camera/rgb/image_raw")
